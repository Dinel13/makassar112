...
export default function BuatWilayah({ cancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await fetch(`${process.env.NEXT_PUBLIC_URL}/phonebook/wilayah/post`, {
        method: "POST",
        headers: {
      ...
      if (!result.ok) {
        console.log(data);
        throw new Error(data.error || "Tidak bisa buat wilayah");
      }
      dispatch(
        showNotif({
          status: "Success",
